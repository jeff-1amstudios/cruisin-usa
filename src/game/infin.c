...
typedef struct INFINITY_POLYGON_ENTRY {
    u32 palette_code;
    u32 texture_addr;
} INFINITY_POLYGON_ENTRY;

static INFINITY_POLYGON_ENTRY BLUESKY[];
...
void INFINITY_CUSA(void) {
    ...
    mode = _MODE & MMODE;
    MAME_ASSERT_REG(0x00008269, "R0", &mode);
    ...
    camera_rady = C3X_LDF(_CAMERARAD.Y);
    // asm 0000826C: 	SETDP
...
LOOP:
    ...
    if ((_MODE & MMODE) == MGAME) {
        ...
        GETLINE_EQ_2D((VECTOR*)&blow_ptr[9], (VECTOR*)&blow_ptr[6], &line);
        // 	;
        // 	;2.  distance to line
        // 	;
        ...
        MAME_ASSERT_REG_FLOAT(0x000082BE, "R0", &HIGHEST_ROADY_X);
...
ok24a:
    ...
    // asm 00008426: 	SETDP
    infinity_height = C3X_LDF(_MATRIXA.a12);
    ...
    TRANS_PTS(26, WATERPOSI, BLOWLIST, horizon_xf, water_y);
    // asm 00008432: 	LDI	wtra_p,AR2
    // asm 00008433: 	CALL	PAL_FIND
    // asm 00008434: 	LDI	R0,AR2
    palette_code = PAL_FIND(wtra_p);
