...
LD_RAM:
    ...
    // asm: STF R0,@_LIGHT+Z
    _LIGHT.Z = C3X_STF_INT(0);
    // asm: LDF 0.707,R0
    // asm: STF R0,@_LIGHT+X
    _LIGHT.X = C3X_STF_IMM(0.707f);
    // asm: STF R0,@_LIGHT+Y
    _LIGHT.Y = C3X_STF_IMM(0.707f);
    // asm 0000933A: 	SETDP
    ...
    // asm: FLOAT R0
    // asm: STF R0,@WHEELPOS
    WHEELPOS = C3X_STF_INT(READADJ(ADJ_STEERCENTER));
    // asm: CLRF R0
    // asm: STF R0,@WHEELPWR
    WHEELPWR = C3X_STF_INT(0);
    // asm: STF R0,@WHEELVEL
    WHEELVEL = 0;
    // asm: CLRI R0
    // asm: STI R0,@COINOFF
...
static void HEAD2HEAD_WAIT(void) {
    ...
    PROC_CONTEXT* ctx = NEW_PROC_CONTEXT();
    CREATE(HEAD2HEAD_LOGO_WAIT, UTIL_C, ctx);
    // asm 0000937C: 	RETS
}
