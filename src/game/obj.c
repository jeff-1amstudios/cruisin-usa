...
static OBJ* OBJ_FIND_FIRST_tail(OBJ* start, int oid);
...
OBJ* OBJ_GET(void) {
    ...
    obj->usr1 = 0;
    obj->usr1_as_float = C3X_STF(C3X_FROM_INT(0));
    ...
    obj->rad.Y = C3X_STF(C3X_FROM_INT(0)); // ;CLEAR RADIANS TO AVOID LOCKUP
    // asm 0000706A: 	STF	R0,*+AR0(ORADZ)
    obj->rad.Z = C3X_STF(C3X_FROM_INT(0));

    // asm 0000706B: 	STF	R0,*+AR0(OMATRIX+1)
    obj->omatrix.mat10 = C3X_STF(C3X_FROM_INT(0));
    // asm 0000706C: 	STF	R0,*+AR0(OMATRIX+2)
    obj->omatrix.mat20 = C3X_STF(C3X_FROM_INT(0));
    // asm 0000706D: 	STF	R0,*+AR0(OMATRIX+3)
    obj->omatrix.mat01 = C3X_STF(C3X_FROM_INT(0));
    // asm 0000706E: 	STF	R0,*+AR0(OMATRIX+5)
    obj->omatrix.mat21 = C3X_STF(C3X_FROM_INT(0));
    // asm 0000706F: 	STF	R0,*+AR0(OMATRIX+6)
    obj->omatrix.mat02 = C3X_STF(C3X_FROM_INT(0));
    // asm 00007070: 	STF	R0,*+AR0(OMATRIX+7)
    obj->omatrix.mat12 = C3X_STF(C3X_FROM_INT(0));

    // asm 00007071: 	LDF	1.0,R0
    // asm 00007072: 	STF	R0,*+AR0(OMATRIX+0)
    obj->omatrix.mat00 = C3X_STF(C3X_FROM_INT(1));
    // asm 00007073: 	STF	R0,*+AR0(OMATRIX+4)
    obj->omatrix.mat11 = C3X_STF(C3X_FROM_INT(1));
    // asm 00007074: 	STF	R0,*+AR0(OMATRIX+8)
    obj->omatrix.mat22 = C3X_STF(C3X_FROM_INT(1));
...
ISCANNXT:
    ...
    if (obj != NULL) {
        obj_vector_x = C3X_SUB(obj->pos.X, _CAMERAPOS.X);
        goto ISCANL;
    }
    // 	;------>BNZD	ISCANL
    // asm 000072C2: 	RETS
...
OBJ* OBJ_QMAKE(void* romdata /*AR2*/, int posx /*R2*/, int posy /*R3*/, int posz /*RC*/) {
    ...
    obj->pos.X = C3X_STF(C3X_FROM_INT(posx));
    // asm 00007339: 	FLOAT	R3
    // asm 0000733A: 	STF	R3,*+AR0(OPOSY)
    obj->pos.Y = C3X_STF(C3X_FROM_INT(posy));
    // asm 0000733B: 	FLOAT	RC,R2
    // asm 0000733C: 	STF	R2,*+AR0(OPOSZ)
    obj->pos.Z = C3X_STF(C3X_FROM_INT(posz));
    // asm 0000733D: 	CLRC
    // asm 0000733E: 	RETS
    return obj;
}
