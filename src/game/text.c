...
TEXTLP:
    ...
    if (color & TXT_CENTER) {
        ...
        // MAME_ASSERT_REG_AT_ADDR(0x00007A85, "R0", &width);
        // asm 00007A86: 	SUBI	R0,R2
        x -= width >> 1;
        // MAME_ASSERT_REG_AT_ADDR(0x00007A87, "R2", &x);
...
NO_CENTER:
    ...
    if (color & TXT_RIGHT) {
        ...
        // MAME_ASSERT_REG_AT_ADDR(0x00007A8B, "R0", &width);
...
IBO2:
    ...
    _ACMAP = text->palette;
    // MAME_ASSERT_REG_AT_ADDR(0x00007AA6, "R1", &_ACMAP);
    ...
    x += pre;
    // MAME_ASSERT_REG_AT_ADDR(0x00007AAD, "R2", &x);
    // MAME_ASSERT_REG_AT_ADDR(0x00007AAD, "R0", &pre);
    ...
    _AIVI[0] = top_left;
    // MAME_ASSERT_REG_AT_ADDR(0x00007AB2, "R0", &top_left);
    ...
    // MAME_ASSERT_REG_AT_ADDR(0x00007ABD, "R1", &_AIVI[2]);

    // asm 00007ABD: 	LDI	*+AR4(TEXT_IMG),R0
    // asm 00007ABE: 	STI	R0,@_ADDRL
    _ADDRL = text->image_addr;
    // MAME_ASSERT_REG_AT_ADDR(0x00007ABF, "R0", &_ADDRL);
    ...
    _ARPS[0] = x;
    _ARPS[9] = x;
    // MAME_ASSERT_REG_AT_ADDR(0x00007AC3, "R2", &x);
    ...
    _ARPS[3] = x;
    _ARPS[6] = x;
    // MAME_ASSERT_REG_AT_ADDR(0x00007AC6, "R2", &x);
    ...
    _ARPS[1] = y;
    _ARPS[4] = y;
    // MAME_ASSERT_REG_AT_ADDR(0x00007AC9, "R3", &y);
    ...
    _ARPS[7] = y;
    _ARPS[10] = y;
    // MAME_ASSERT_REG_AT_ADDR(0x00007ACC, "R3", &y);
