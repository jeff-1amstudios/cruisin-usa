...
c3x_f32_t LEG_RADY = C3X_F32_INIT(1.0f);
...
static LEG_PAYLOAD* LEG_ADD_GROUP(tyco_stream_t tyco_ptr /*AR2*/, LEG_PAYLOAD* map_entry /*AR3*/, int section_index /*AR4*/) {
    ...
    LEG_RADY = C3X_STF(C3X_FROM_RAW32(tyco_ptr[4]));
    ...
    HPFIND_YMATRIX(&MATRIXAI, C3X_LDF(LEG_RADY));
...
L12:
    ...
    leg->as_float.pos.X = C3X_STF_INT(*group_ptr++);
    // asm 0000AAEB: 	FLOAT	*AR5++,R1		;GET Y POSITION
    leg->as_float.pos.Y = C3X_STF_INT(*group_ptr++);
    ...
    leg->as_float.pos.Z = C3X_STF_INT(*group_ptr++);
...
ISOVER:
    ...
    leg->as_float.pos.X =C3X_STF(C3X_ADD(leg->as_float.pos.X, C3X_FROM_RAW32(tyco_ptr[7])));
    ...
    leg->as_float.pos.Y =C3X_STF(C3X_ADD(leg->as_float.pos.Y, C3X_FROM_RAW32(tyco_ptr[8])));
    // asm 0000AAFB: 	LDF	*+AR7(TB_RVS_POSZ),R0
    // asm 0000AAFC: 	ADDF	*+AR3(LEG_POSZ),R0
    // asm 0000AAFD: 	STF	R0,*+AR3(LEG_POSZ)
    leg->as_float.pos.Z =C3X_STF(C3X_ADD(leg->as_float.pos.Z, C3X_FROM_RAW32(tyco_ptr[9])));
    if ((flag & SC_OVERLAY) == 0) {
        tyco_ptr += 1;
    }
...
ISOVERA:
    ...
    leg->as_float.pos.X =C3X_STF(C3X_ADD(VECTORAI.X, C3X_FROM_RAW32(tyco_ptr[1])));
    ...
    leg->as_float.pos.Y =C3X_STF(C3X_ADD(VECTORAI.Y, C3X_FROM_RAW32(tyco_ptr[2])));
    // asm 0000AB0B: 	LDF	*AR0++,R1
    // asm 0000AB0C: 	ADDF	*+AR7(TB_POSZ),R1
    // asm 0000AB0D: 	STF	R1,*+AR3(LEG_POSZ)
    leg->as_float.pos.Z =C3X_STF(C3X_ADD(VECTORAI.Z, C3X_FROM_RAW32(tyco_ptr[3])));
...
JOIN_UP:
    ...
    object_radians = C3X_FROM_RAW32(*group_ptr++);
...
NOTRVSTRK:
    ...
    object_radians = C3X_LDF(leg->as_float.pos.Z);
