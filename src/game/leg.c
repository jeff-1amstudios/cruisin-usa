...
static int ELEMENT_GET(void);
static void ELEMENT_ADD(int element_offset);
static LEG_MAP_ENTRY* ELEMENT_DUMP_INTO_LEGMAP(LEG_MAP_ENTRY* map_entry);
void LEG_GENERATE_MAP(int start_index /*AR0*/, int end_index /*AR1*/);
static LEG_MAP_ENTRY* LEG_ADD_GROUP(u32* tyco, LEG_MAP_ENTRY* map_entry, int section_index);
...
static int ELEMENT_GET(void) {
    ...
    element_offset = LEGFREE;
    // asm 0000AA72: 	LDI	AR3,AR0
    LEGFREE = element_offset + LEG_SIZE + 1;
...
static void ELEMENT_ADD(int element_offset) {
    ...
    element_id = LEG_SSLL[element_offset - 1 + LEG_ENTRY_WORD + 3];
    // asm 0000AA79: 	LDI	@LEGLLI,AR0
    link_offset_ptr = &LEGLLI;
R65:
    // asm 0000AA7A: LDI	*AR0,AR1
    next_offset = *link_offset_ptr;
    // asm 0000AA7B: 	CMPI	0,AR1
    if (next_offset != 0) {
        // asm 0000AA7C: 	BEQ	QT
        // asm 0000AA7D: 	CMPI	*+AR1(1+LEG_ID),R0
        if (LEG_SSLL[next_offset - 1 + LEG_ENTRY_WORD + 3] <= element_id) {
            // asm 0000AA7E: 	BLT	QT
            // asm 0000AA7F: 	LDI	AR1,AR0
            link_offset_ptr = &LEG_SSLL[next_offset - 1 + LEG_LINK_WORD];
            // asm 0000AA80: 	BU	R65
            goto R65;
        }
    }
QT:
    ...
    LEG_SSLL[element_offset - 1 + LEG_LINK_WORD] = next_offset;
    // asm 0000AA82: 	STI	AR3,*AR0
    *link_offset_ptr = element_offset;
    // asm 0000AA83: 	RETS
}
...
EDILL:
    ...
    element_offset = LEG_SSLL[element_offset - 1 + LEG_LINK_WORD];
...
K99:
    ...
    *map_entry++ = *(LEG_MAP_ENTRY*)&LEG_SSLL[current_offset - 1 + LEG_ENTRY_WORD];
...
void LEG_GENERATE_MAP(int start_index /*AR0*/, int end_index /*AR1*/) {
    u32* tyco_ptr;
    int section_index;
    LEG_MAP_ENTRY* map_entry;
    u32 flag;

    // asm 0000AA96: 	LDI	AR1,R1
    TRACE_EVENT(&g_crusn_machine->trace, "function", "LEG_GENERATE_MAP", 0, 0);
    MAME_ASSERT_REG_AT_ADDR(0x0000AA96, "AR0", &start_index);
    MAME_ASSERT_REG_AT_ADDR(0x0000AA96, "AR1", &end_index);
...
LPP:
    ...
    flag = *tyco_ptr;
    tyco_ptr += 6;
    MAME_ASSERT_REG_AT_ADDR(0x0000AA9E, "R1", &flag);
...
DNFND:
    ...
    map_entry = (LEG_MAP_ENTRY*)LEG_MAPI;
    // *
...
LOAD_LP:
    ...
    MAME_ASSERT_REG_AT_ADDR(0x0000AAAA, "AR4", &section_index);
...
LOAD_LP2:
    ...
    MAME_ASSERT_REG_AT_ADDR(0x0000AABB, "AR4", &section_index);
    map_entry = LEG_ADD_GROUP(tyco_ptr, map_entry, section_index);
...
CNTNB2:
    ...
    map_entry->pos_y_with_lane_flag = -1;
    map_entry->pos_z = -1;
    map_entry->id = -1;
 ...
 *
 */
static LEG_MAP_ENTRY* LEG_ADD_GROUP(u32* tyco_ptr /*AR2*/, LEG_MAP_ENTRY* map_entry /*AR3*/, int section_index /*AR4*/) {
    int flag;
    int lane_count;
    ...
    printf("section %d, lanes %d, radians %f\n", section_index, lane_count, LEG_RADY);
    // asm 0000AAE1: 	STF	R2,@LEG_RADY
    MAME_ASSERT_REG_AT_ADDR_FLOAT(0x0000AAE1, "R2", &LEG_RADY);
...
L12:
    ...
    element_entry = (LEG_ASM*)&LEG_SSLL[element_offset - 1 + LEG_ENTRY_WORD];
    (void)*group_ptr++;
    // asm 0000AAE9: 	FLOAT	*AR5++,R1		;GET X POSITION
    element_entry->pos.X = (float)(int)*group_ptr++;
    // asm 0000AAEA: 	STF	R1,*+AR3(LEG_POSX)
    // asm 0000AAEB: 	FLOAT	*AR5++,R1		;GET Y POSITION
    element_entry->pos.Y = (float)(int)*group_ptr++;
    ...
    element_entry->pos.Z = (float)(int)*group_ptr++;
...
NOTREVERSED:
    ...
    MATRIX_MUL((VECTOR*)&element_entry->pos.X, &MATRIXAI, &VECTORAI);
...
JOIN_UP:
    ...
    object_id = (int)*group_ptr++;
    // asm 0000AB21: 	LDI	R1,R0
    // asm 0000AB22: 	AND	00F00h,R0
    if ((object_id & 0x0F00) != 0x0B00) {
        goto NOTDYNAROAD;
    }
