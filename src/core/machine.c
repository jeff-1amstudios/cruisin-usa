...
enum {
    CRUSN_DMA_COLOR_MASK = 0x00ff,
    CRUSN_DMA_TM = 0x0100,
    CRUSN_DMA_NZR = 0x0400,
    CRUSN_DMA_ZS = 0x0800,
    CRUSN_DMA_DITHER = 0x2000,
};
...
int crusn_machine_draw_fpga(
    ...
    textured = (control & CRUSN_DMA_TM) != 0;
    zero_suppress = (control & CRUSN_DMA_ZS) != 0;
    nonzero_replace = (control & CRUSN_DMA_NZR) != 0;
    dither = (control & CRUSN_DMA_DITHER) != 0;
    write_page_words = crusn_machine_screen_page(g_crusn_machine, g_crusn_machine->write_page_index);
    ...
    for (int y = dest_min_y; y <= dest_max_y; ++y) {
        ...
        for (int x = dest_min_x; x <= dest_max_x; ++x) {
            ...
            if (!textured) {
                color_index = (u32)(palette_base + (control & CRUSN_DMA_COLOR_MASK));
            } else {
