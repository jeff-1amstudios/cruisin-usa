...
static uint32_t dcs_master_gain(int volume) {
    ...
    for (int bit = 0; bit < 8; ++bit) {
        if ((value & 1u) == 0) x = (uint16_t)(((uint32_t)x * y) >> 15);
    ...
    for (int out = 0; out < frames; ++out) {
        ...
        for (int channel = 0; channel < CRUSN_AUDIO_CHANNELS; ++channel) {
            ...
            const size_t next = position + 1 < voice->track->sample_count
                ? position + 1 : (voice->track->looping ? 0 : position);
...
void crusn_audio_shutdown(void) {
    if (g_audio.device != 0) SDL_CloseAudioDevice(g_audio.device);
    dcs_audio_free(g_audio.bank);
    memset(&g_audio, 0, sizeof(g_audio));
}

void crusn_audio_set_master_volume(int volume) {
    if (g_audio.device == 0) return;
    SDL_LockAudioDevice(g_audio.device);
    g_audio.master_gain = dcs_master_gain(volume);
    SDL_UnlockAudioDevice(g_audio.device);
}

void crusn_audio_set_channel_volume(int channel, int volume) {
    if (g_audio.device == 0 || channel < 0 || channel >= CRUSN_AUDIO_CHANNELS) return;
    SDL_LockAudioDevice(g_audio.device);
    g_audio.channel_volume[channel] = clamp_u8(volume);
    SDL_UnlockAudioDevice(g_audio.device);
}

static void play_sound_on_channel(int channel, int sound_index, int volume,
                                  const crusn_sound_entry* entry) {
    if (entry->dcs_track == 0) {
        memset(g_audio.voices, 0, sizeof(g_audio.voices));
        for (int i = 0; i < CRUSN_AUDIO_CHANNELS; ++i) g_audio.voices[i].sound_index = -1;
    } else if (entry->dcs_track >= 995 && entry->dcs_track <= 998) {
        channel = (int)(entry->dcs_track - 995);
        memset(&g_audio.voices[channel], 0, sizeof(g_audio.voices[channel]));
        g_audio.voices[channel].sound_index = -1;
    } else if (channel >= 0 && channel < CRUSN_AUDIO_CHANNELS) {
        crusn_audio_voice* voice = &g_audio.voices[channel];
        voice->track = dcs_audio_track(g_audio.bank, entry->dcs_track);
        voice->position = 0;
        voice->sound_index = sound_index;
        voice->priority_word = entry->priority_word;
        voice->volume = clamp_u8(volume);
    }
}

void crusn_audio_play_sound(int channel, int sound_index, int volume) {
    const crusn_sound_entry* entry = crusn_sound_entry_from_index(sound_index);
    if (entry == NULL || g_audio.device == 0
        || channel < 0 || channel >= CRUSN_AUDIO_CHANNELS) return;

    SDL_LockAudioDevice(g_audio.device);
    play_sound_on_channel(channel, sound_index, volume, entry);
    SDL_UnlockAudioDevice(g_audio.device);
}

int crusn_audio_kill_sound(int sound_index) {
    int killed = 0;
    if (g_audio.device == 0) return 0;
    SDL_LockAudioDevice(g_audio.device);
    for (int channel = 1; channel <= 2; ++channel) {
        if (g_audio.voices[channel].sound_index == sound_index) {
            memset(&g_audio.voices[channel], 0, sizeof(g_audio.voices[channel]));
            g_audio.voices[channel].sound_index = -1;
            killed = 1;
        }
...
void crusn_audio_play_track(int dcs_track) {
    ...
    if (dcs_track == 0) {
        memset(g_audio.voices, 0, sizeof(g_audio.voices));
    } else if (dcs_track == 1000) {
        g_audio.engine.source_increment = 127;
        g_audio.engine.source_threshold = 255;
        g_audio.engine.primed = 0;
    } else {
        const dcs_pcm_track* track = dcs_audio_track(g_audio.bank, (unsigned)dcs_track);
        if (track != NULL) {
            g_audio.voices[0].track = track;
            g_audio.voices[0].position = 0;
            g_audio.voices[0].sound_index = -1;
            g_audio.voices[0].priority_word = 0;
            g_audio.voices[0].volume = 255;
        }
    }
    SDL_UnlockAudioDevice(g_audio.device);
}
...
void crusn_audio_stop_all(void) {
    ...
    memset(g_audio.voices, 0, sizeof(g_audio.voices));
    for (int i = 0; i < CRUSN_AUDIO_CHANNELS; ++i) g_audio.voices[i].sound_index = -1;
    g_audio.engine.source_increment = 0;
    g_audio.engine.volume = 0;
    SDL_UnlockAudioDevice(g_audio.device);
}

void crusn_audio_set_player_engine(int speed, int volume) {
    if (g_audio.device == 0 || g_audio.engine.track == NULL) return;
