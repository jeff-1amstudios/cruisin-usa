...
dcsGameInfo[] ={
	...
	{ DCSDecoder::GameID::CRUSNUSA, "Cruis'n USA", "Cruis'n USA (c)1994 Midway" },
...
uint32_t DCSDecoder::FindCatalog(const uint8_t *u2, size_t u2size)
{
	...
	// Most pinball titles use one of the first three locations.  Cruis'n USA
	// uses the same catalog format at $24000.
	static const uint32_t offsets[] ={ 0x3000, 0x4000, 0x6000, 0x24000 };
	for (int i = 0 ; i < static_cast<int>(_countof(offsets)); ++i)
	{
		if (offsets[i] + 6 > u2size)
			continue;
